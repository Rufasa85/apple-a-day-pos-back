...
const generateSeedData = (UserId) => {
	...
	for (let i = 0; i < numOrders; i++) {
		const randomCustomerId = i % 2 === 0 ? null : faker.number.int({ min: 1, max: numCustomers }) + (UserId - 1) * numCustomers;
		const randomShiftId = faker.number.int({ min: 1, max: numShifts }) + (UserId - 1) * numShifts;
		...
		for (let j = 0; j < numItemsPerOrder; j++) {
			const itemIds = shiftItems.filter((shiftItem) => shiftItem.ShiftId === randomShiftId);
